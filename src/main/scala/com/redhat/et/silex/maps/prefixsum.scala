...
object tree {
  ...
  trait INodePS[K, V, P] extends NodePS[K, V, P] with INodeMap[K, V] {
    ...
    override def toString = s"INodePS(${data.key},${data.value},$prefix)"
  }
}

import com.redhat.et.silex.maps.redblack.tree.Node
import com.redhat.et.silex.maps.ordered.tree.NodeMap
import tree._
...
object infra {
  ...
  class Inject[K, V, P](val keyOrdering: Ordering[K], val prefixMonoid: IncrementingMonoid[P, V]) {
    def iNode(clr: Color, dat: Data[K], ls: Node[K], rs: Node[K]) =
      new Inject[K, V, P](keyOrdering, prefixMonoid) with INodePS[K, V, P] {
        // INode[K]
        val color = clr
...
/** An ordered map that supports a prefix-sum query
  ...
  * @tparam P The prefix-sum type
  */
case class PrefixSumMap[K, V, P](root: NodePS[K, V, P]) extends
    PrefixSumMapLike[K, V, P, INodePS[K, V, P], PrefixSumMap[K, V, P]] {

  def build(n: Node[K]) = PrefixSumMap(n.asInstanceOf[NodePS[K, V, P]])
  ...
  def key[K](implicit ord: Ordering[K]) = new AnyRef {
    def value[V] = new AnyRef {
      def prefix[P](implicit mon: IncrementingMonoid[P, V]) =
        PrefixSumMap(new Inject[K, V, P](ord, mon) with LNodePS[K, V, P])
    }
  }
}

...
object tree {
  ...
  trait Node[K] {
    ...
    final def insert(d: Data[K]) = blacken(ins(d))
    ...
    // internal
    def ins(d: Data[K]): Node[K]
    def del(k: K): Node[K]

    protected
    def iNode(color: Color, d: Data[K], lsub: Node[K], rsub: Node[K]): INode[K]
  ...
  trait LNode[K] extends Node[K] {
    ...
    def ins(d: Data[K]) = rNode(d, this, this)
    def del(k: K) = this
  }
  ...
  trait INode[K] extends Node[K] {
    ...
    override def toString = s"INode(${data.key})"
  }

  ...
  object infra {
    ...
    class Inject[K, V, P](
      ...
      def iNode(clr: Color, dat: Data[K], ls: Node[K], rs: Node[K]) =
        new Inject[K, V, P](keyOrdering, valueMonoid, prefixMonoid) with INodeTD[K, V, P] {
          // INode[K]
          val color = clr
    ...
    trait TDigestMapLike[K, V, P, IN <: INodeTD[K, V, P], M <: TDigestMapLike[K, V, P, IN, M]]
      ...
      val root: NodeTD[K, V, P]
    }
  }

  import infra._

  case class TDigestMap[K, V, P](root: NodeTD[K, V, P])
      extends TDigestMapLike[K, V, P, INodeTD[K, V, P], TDigestMap[K, V, P]] {

    def build(n: Node[K]) = TDigestMap(n.asInstanceOf[NodeTD[K, V, P]])
    ...
    def key[K](implicit num: Numeric[K]) = new AnyRef {
      def value[V](implicit vm: Monoid[V]) = new AnyRef {
        def prefix[P](implicit im: IncrementingMonoid[P, V]) =
          TDigestMap(new Inject[K, V, P](num, vm, im) with LNodeTD[K, V, P])
      }
    }
  }
}
